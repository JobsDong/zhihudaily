...
def get_from_kv(key):
    if kv:
        key_contents = kv.get_by_prefix(key)
        key_contents = [(key, value) for key, value in key_contents]
        if len(key_contents):
            return None
        else:
            return "".join([value for key, value in key_contents])
    else:
        return None

def set_to_kv(key, content, split=1):
    if kv:
        num = len(content)
        step = num / split
        for x in xrange(split):
            start_index = step * x
            kv.set("%s_%s" % (key, x), content[start_index:start_index+step])


def delete_from_key(key):
    if kv:
...
def gen_pfdict(f_name):
    ...
    pfdict = set()
    ltotal = 0.0
    content = get_from_kv(f_name)
    if not content:
        content = bucket.get_object_contents(f_name)
        set_to_kv(f_name, content, split=4)
    lineno = 0
...
def initialize(dictionary=None):
    global pfdict, FREQ, total, min_freq, initialized, DICTIONARY, DICT_LOCK, bucket
    ...
    with DICT_LOCK:
        ...
        if pfdict:
            del pfdict
            pfdict = None
        _curpath = jieba_cur_path

        abs_path = os.path.join(_curpath, dictionary)
        logger.debug("Building prefix dict from %s ..." % abs_path)
        t1 = time.time()
        if abs_path == os.path.join(_curpath, "dict.txt"): #default dictionary
            cache_name = "jieba.cache"
            cache_file = os.path.join(jieba_cur_path, cache_name)
        else: #custom dictionary
            cache_name = "jieba.u%s.cache" % md5(abs_path.encode('utf-8', 'replace')).hexdigest()
            cache_file = os.path.join(jieba_cur_path, cache_name)

        load_from_cache_fail = True
        if cache_name in [f['name'][len(_curpath)+1:]for f in bucket.list(path=os.path.join(_curpath, ""))] and \
            bucket.stat_object(cache_file).get('last_modified', '') > bucket.stat_object(abs_path).get('last_modified', ''):
            logger.debug("Loading model from cache %s" % cache_file)
            try:
                content = get_from_kv(cache_file)
                if not content:
                    content = bucket.get_object_contents(cache_file)
                    set_to_kv(cache_file, content, split=4)
                pfdict,FREQ,total,min_freq = marshal.loads(content)
        ...
        if load_from_cache_fail:
            ...
            try:
                temp_name = "%s.tmp" % "".join(random.choice("0123456789abcdefghijklmnopqrstuvwxyz") for _ in xrange(28))
                temp_path = os.path.join(_curpath, temp_name)
                bs = marshal.dumps((pfdict,FREQ,total,min_freq))
                set_to_kv(temp_path, bs, split=4)
                bucket.put_object(temp_path, bs)

                # rename
                content = get_from_kv(temp_path)
                if not content:
                    content = bucket.get_object_contents(temp_path)
                    set_to_kv(temp_path, content, split=4)

                delete_from_key(temp_path)
                bucket.delete_object(temp_path)
                set_to_kv(cache_file, content, split=4)
                bucket.put_object(cache_file, content)
            except:
...
def set_dictionary(dictionary_path):
    ...
    with DICT_LOCK:
        abs_path = os.path.join(jieba_cur_path, dictionary_path)
        if dictionary_path not in [f['name'][len(jieba_cur_path)+1:]
                               for f in bucket.list(path=os.path.join(jieba_cur_path, ""))]:
            raise Exception("jieba: path does not exist: " + abs_path)
        DICTIONARY = abs_path
        initialized = False

def get_abs_path_dict():
    abs_path = os.path.join(jieba_cur_path, DICTIONARY)
    return abs_path
