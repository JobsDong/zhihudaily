...
def load_model(f_name, isJython=True):
    _curpath = saejieba.jieba_cur_path

    result = {}
    f = saejieba.bucket.get_object_contents(f_name)
    for line in f.split("\n"):
        line = line.strip()
        if not line:
            continue
    ...
    f = saejieba.bucket.get_object_contents(abs_path)
    start_p = marshal.loads(f)

    trans_p = {}
    abs_path = os.path.join(_curpath, PROB_TRANS_P)
    f = saejieba.bucket.get_object_contents(abs_path)
    trans_p = marshal.loads(f)

    emit_p = {}
    abs_path = os.path.join(_curpath, PROB_EMIT_P)
    f = saejieba.bucket.get_object_contents(abs_path)
    emit_p = marshal.loads(f)

    state = {}
    abs_path = os.path.join(_curpath, CHAR_STATE_TAB_P)
    f = saejieba.bucket.get_object_contents(abs_path)
    state = marshal.loads(f)

    return state, start_p, trans_p, emit_p, result

if sys.platform.startswith("java"):
    char_state_tab_P, start_P, trans_P, emit_P, word_tag_tab = load_model(saejieba.get_abs_path_dict())
else:
    ...
    word_tag_tab = load_model(saejieba.get_abs_path_dict(), isJython=False)

def makesure_userdict_loaded(fn):

    @wraps(fn)
    def wrapped(*args,**kwargs):
        if saejieba.user_word_tag_tab:
            word_tag_tab.update(saejieba.user_word_tag_tab)
            saejieba.user_word_tag_tab = {}
...
def __cut_DAG_NO_HMM(sentence):
    DAG = saejieba.get_DAG(sentence)
    route = {}
    saejieba.calc(sentence, DAG, route)
    x = 0
    N = len(sentence)
    buf = u''
...
def cut(sentence, HMM=True):
    if (not hasattr(saejieba, 'pool')) or (saejieba.pool is None):
        for w in __cut_internal(sentence, HMM=HMM):
            yield w
    else:
        parts = re.compile('([\r\n]+)').split(sentence)
        if HMM:
            result = saejieba.pool.map(__lcut_internal, parts)
        else:
            result = saejieba.pool.map(__lcut_internal_no_hmm, parts)
        for r in result:
            for w in r:
                yield w
